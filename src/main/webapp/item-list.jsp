...
<%
    ...
    bids = (List<Bid>) request.getAttribute("bids");
    User user = (User) session.getAttribute("user");
    bidInfo = (HashMap<String, Integer>) request.getAttribute("bidInfo");
%>
<!DOCTYPE html>
<html>
<head>
...
<div class="main-container">
  <div class="left-component">
  	...
  	<form class="serach-container" action="items" method="get">
    ...
    <div class="history">
        <div class="history-row">
            <p>Spent</p> <p><%=bidInfo.get("expend")%>$</p>
        </div>
        <div class="history-row">
            <p>Bidded items</p><p> <%=bidInfo.get("totalBids")%></p>
        </div>
        <div class="history-row">
            <p>Acquired items</p> <p> <%=bidInfo.get("paidBids")%></p>
        </div>
        <div class="history-row">
            <p>Pending items </p> <p><%=bidInfo.get("pendingBids")%></p>
        </div>
    </div>

    <div class="sub-topic">
      <div class="w-[100%] flex justify-between items-center">
          <h3>ITEMS YOU BID ON</h3>
          <i class="icon-arrow"></i>
      </div>
      <div class="sub-topic-content">
          <% for (Bid bid : bids) { %>
            <div class="small-card">
                <img src="<%=bid.getImageUrl()%>" alt="Item Image">
                <p><%= bid.getItemTitle() %></p>
                <p>$<%= bid.getAmount()%></p>
            </div>
        <% } %>
      </div>
    ...
    <div class="sub-topic">
        <div class="w-[100%] flex justify-between items-center">
            <h3>ITEMS YOU ACQUIRED</h3>
            <i class="icon-arrow"></i>
        </div>
        <div class="sub-topic-content">
          <% for (Bid bid : bids) { %>
              <% if (bid.isWinner() && bid.isPaidBidder()) { %>
                  <div class="small-card">
                      <img src="<%=bid.getImageUrl()%>" alt="Item Image">
                      <p><%=bid.getItemTitle()%></p>
                      <p>$<%= bid.getAmount()%></p>
    ...
    <div class="sub-topic">
        <div class="w-[100%] flex justify-between items-center">
            <h3>Items Pending for a Payment</h3>
            <i class="icon-arrow"></i>
        </div>
        <div class="sub-topic-content" >
          ...
          <div class="small-card">
              ...
              <form method="POST" action="https://api.chapa.co/v1/hosted/pay" >
                  ...
                  <input type="hidden" name="tx_ref" value="<%= "negade-tx-12345678leu" + generateRandom5DigitNumber() %>" />
                  ...
                  <input type="hidden" name="return_url" value="<%="http://localhost:8080/BiddingSystem/bidder-paid?bidId="+ bid.getId() %>" />
                  <input type="hidden" name="meta[title]" value="test" />
                  <button class="pay-button" type="submit">Pay</button>
              </form>
<%--              <form action="bidder-paid" method="post">--%>
...
<%--              </form>--%>
  ...
  <div class="right-component" style="display: flex; flex-direction: column; align-items: center;">
      <%
          ...
          if (searchParam != null && items.size() > 0) {
      %>
      <p style="margin-bottom: 20px">Search Result For <%=searchParam%></p>
      <%
          } else if (searchParam == null && items.size() > 0) {
      %>
    ...
    <div class="card">
      ...
      <div class="card-details">
        <div class="top-desc">
          <h3><%= item.getTitle()%></h3>
          <p class="end-date"><%= item.getEndDate()%></p>
        </div>
          <p><%=item.getDescription()%></p>
          <div class="bottom-desc">
          <p>Max Bid: $<%=item.getMaxAmount()%></p>
          <form action="bid-redirect" method="post">
              <input type="hidden" name="itemId" value="<%=item.getId()%>">
              <button type="submit" class="bid-button">Bid</button>
          </form>
          <form action="user-item-detail" method="get">
              <input type="hidden" name="itemId" value="<%=item.getId()%>">
              <button type="submit" class="bid-button">View detail</button>
          </form>
        </div>
      </div>
    </div>
...
<script>
    ...
    // JavaScript to toggle sub-topic content
    const subTopics = document.querySelectorAll('.sub-topic');
    subTopics.forEach(subTopic => {
        const subTopicContent = subTopic.querySelector('.sub-topic-content');
        const heading = subTopic.querySelector('h3');
        const icon = subTopic.querySelector(".icon-arrow");
        
        // Add click event listener to the heading
        heading.addEventListener('click', () => {
            subTopicContent.classList.toggle('show');
